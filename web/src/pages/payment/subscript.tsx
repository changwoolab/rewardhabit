...
const subscript: React.FC<subscriptProps> = ({}) => {
    ...
    const [packageValues, setPackageValues] = useState({
        type: 0, target: 0, dailyRefund: 0, term: 0, payment: 0
    });
    
    // 여러번 적기 싫어서 정의
    const packageButton = (
      title: string, packageName: string, fullPrice: string, price: string, subscriptTerm: string, refund: string, extra: string,
      variables: {
        type: number, target: number, dailyRefund: number, term: number, payment: number
      }
    ) => {
      return (
        <Button h={"100%"} w={"100%"} flex={1} m={"2px"} colorScheme={"teal"} variant={"outline"} onClick={() => {
          ...
          <Box w={"100%"}>
            <Box m={4}>
              <Text color={color} fontSize={"xx-large"}>{title}</Text>
            </Box>
            <hr className={hrStyle}/>
            <Box mt={4}>
              <Text color={color} fontSize={"lg"}>{packageName}</Text>
            </Box>
            <Box mt={4} mb={4}>
              <Text as="del" color="gray">정가: {fullPrice}원</Text>
              <strong><Text color={color} fontSize={"xx-large"}>{price}원</Text></strong>
            </Box>
            <hr className={hrStyle}/>
            <Box mt={4}>
              <Text color={color}>구독 기간</Text>
              <strong><Text mt={1} mb={4} color={color} fontSize={"lg"}>{subscriptTerm}일</Text></strong>
              <Text color={color}>일당 환급 액수</Text>
              <strong><Text mt={1} mb={4} color={color} fontSize={"lg"}>{refund}원</Text></strong>
              <Text color={color}>추가 특별 보상</Text>
              <strong><Text mt={1} mb={4} color={color} fontSize={"lg"}>{extra}회</Text></strong>
            </Box>
          </Box>
      </Button>
      )
    ...
    return (
        <Layout variant="large">
          <Flex justifyContent={"center"}>
            {packageButton(
              "PACKAGE 1", "31일 습관 0원 패키지", "310,000", "299,000", "31", "10,000", "1",
              {
                type: 1, target: 0, dailyRefund: 10000, term: 31, payment: 299000
              }
            )}
            {packageButton(
                "PACKAGE 2", "62일 습관 0원 패키지", "620,000", "599,000", "62", "10,000", "2",
                {
                  type: 2, target: 0, dailyRefund: 10000, term: 62, payment: 599000
                }
            )}
            {packageButton(
                "PACKAGE 3", "93일 습관 0원 패키지", "930,000", "899,000", "93", "10,000", "3",
                {
                  type: 3, target: 0, dailyRefund: 10000, term: 93, payment: 899000
                }
            )}
          </Flex>
                ...
                <Box mt={4} borderWidth='1px' borderRadius='lg' overflow='hidden' borderColor={color}>
                  <Flex m={4} justifyContent={"center"}>
                    <SubscriptBox desc="선택한 패키지">
                      <InputField width={"80%"} name="package" value={packageSelectOptions[packageValues.type]} readOnly/>
                    </SubscriptBox>
