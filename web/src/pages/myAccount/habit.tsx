import { Box, Button, Flex, Grid, GridItem, Heading, Link, Stack, Text, useColorMode } from '@chakra-ui/react';
...
const habit: React.FC<habitProps> = ({}) => {
  ...
  const timeTableHours = () => {
    ...
    for (let i = 0; i < 24; i++) {
      li.push(<Flex justifyContent={"center"} alignItems={"center"} h={`${heightPerHour}px`}>{i}</Flex>)
    }
    return (<>{li}</>)
  }
  ...
  const dayTimeGenerator = (type: string) => {
    ...
    myHabits.myHabits.forEach((value) => {
      ...
      if (value.habitDay.includes(type)) habits.push({
        ...value, 
        start,
        end,
      });
    ...
    let index = 0;
    while (time < 24) {
      if (index < sortedHabits.length && Math.floor(sortedHabits[index].start) == Math.floor(time)) {
        let gap = sortedHabits[index].end - sortedHabits[index].start;
        li.push(
          <Flex direction="column" justifyContent={"center"} alignItems={"center"} h={`${heightPerHour*gap}px`} border="1px" borderColor={color}>
            <Text fontSize={"5px"}>{sortedHabits[index].habitName}</Text>
            <Text fontSize={"3px"}>{sortedHabits[index].habitStart}~{sortedHabits[index].habitEnd}</Text>
          </Flex>
          )
        index++;
        time += gap;
        ...
        li.push(<Flex justifyContent={"center"} alignItems={"center"} h={`${heightPerHour*0.5}px`} borderColor={"white"}></Flex>)
        time += 0.5;
      } else {
        li.push(<Flex justifyContent={"center"} alignItems={"center"} h={`${heightPerHour}px`} borderColor={"white"}></Flex>)
        time += 1;
      }
    }
    return (<>{li}</>)
  ...
  const habitValidationSchema = object().shape({
    habitStart: string().test("habitStart", "정확히 입력하세요 예)05:30", checker), 
    ...
    return (
        <MyAccountLayout>
                  ...
                  <Box key={p.id} p={2} shadow="md" borderwidth="1px">
                    <Flex>
                      <Box flex={1}>
                        ...
                        <Text fontSize="sm" ml={4}>종료시간: {p.habitEnd}</Text>
                        </Flex>
                      </Box>
                    </Flex>
                  </Box>
          ...
          <Box textAlign={"center"}>
            ...
            <Text>분은 30분 단위로만 입력 가능합니다. 예시) 00:30(O), 00:20(X)</Text>
