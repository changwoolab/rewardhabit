...
const myPost: React.FC<myPostProps> = ({}) => {
  ...
  const [optionState, setOptionState] = useState<"show-option" | "hide-option">(
    "hide-option"
  );
  ...
  const [{ data, fetching }] = useOffsetBasePostsQuery({
    variables: { type, limit, page },
  });
    ...
    <MyAccountLayout>
      <Box textAlign={"center"} flex={1}>
        ...
        {optionState === "hide-option" ? null : (
          <Box>
            <Formik
              initialValues={{ type1: "", type2: "", type3: "", limit: limit }}
              onSubmit={(value) => {
                ...
                router.push(
                  `/myAccount/myPost?page=1&limit=${value.limit}&type=${typeInfo}`
                );
              }}
            >
              {({ isSubmitting }) => (
                <Form>
                  <Flex justifyContent={"center"}>
                    ...
                    <Field name="type2">
                      {({ field }: any) => (
                        <Checkbox ml={4} {...field} key="1" value="2">
                          독서록
                        </Checkbox>
                      )}
                    </Field>
                    <Field name="type3">
                      {({ field }: any) => (
                        <Checkbox ml={4} {...field} key="1" value="3">
                          AI질문게시판
                        </Checkbox>
                      )}
                    </Field>
