...
export class PostResolver {
  ...
  textsSnippet(@Root() root: Post) {
    return root.texts.slice(0, 50);
  }
  /* 다른 사람이 내 독서록/일기 못보게 함 */
  @FieldResolver(() => User, { nullable: true })
  user(@Root() post: Post, @Ctx() { req, userLoader }: ReqResContext) {
  ...
  title(@Root() post: Post, @Ctx() { req }: ReqResContext) {
  ...
  @FieldResolver(() => String)
  texts(@Root() post: Post, @Ctx() { req }: ReqResContext) {
  ...
  @FieldResolver(() => String)
  writtenDate(@Root() post: Post, @Ctx() { req }: ReqResContext) {
  ...
  @FieldResolver(() => String)
  updateDate(@Root() post: Post, @Ctx() { req }: ReqResContext) {
  ...
  @FieldResolver(() => String)
  updoots(@Root() post: Post, @Ctx() { req }: ReqResContext) {
  ...
  async offsetBasePosts(
    ...
    const posts = (await getConnection()
      .createQueryBuilder()
      .from((subQuery) => {
        subQuery.select("*").from(Post, "post");
  ...
  async createPost(
    ...
    if (input.type == 3) {
      ...
      getConnection().transaction(async (tm) => {
        ...
        ); // 회원탈퇴할 때 포스트들 및 댓글도 사라지므로 userId는 올린 사람으로 해도 됨.
        await tm.query(
          `
